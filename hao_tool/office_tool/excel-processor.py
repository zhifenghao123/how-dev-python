...
def read_excel_file_to_generate_sql_insert(excel_file_path):
    sheet_data = pd.read_excel(excel_file_path)
    #print(sheet_data)
    insert_sql = generate_sql_insert_schema()
    for row_value in sheet_data.values:
        #print(concat_generate_sql_insert(row_value))
        insert_sql = insert_sql + concat_generate_sql_insert(row_value)
    print(insert_sql)


def generate_sql_insert_schema():
    return 'INSERT INTO `h_score`(`area`, `score`, `num`, `total_num`) VALUES'


def concat_generate_sql_insert(record):
    return '(\'' + str(record[0]).replace('\n', '') + '\',\'' + str(record[1]).replace('\n', '') + '\',' + str(record[2]) + ',' + str(record[3]) + '),'


if __name__ == '__main__':
    read_excel_file_to_generate_sql_insert('../dataset/ShowFile.xlsx')
